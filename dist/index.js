...
async function handleFiles(files) {
    ...
    for (const { path: path2  } of files){
        result.push({
            data: await Deno.readFile(path2),
            ...parse2(relative2(Deno.cwd(), path2))
        });
    }
    return result;
}
async function dest1(path2, fileDatas) {
    await ensureDir(path2);
    for (const fileData of fileDatas){
        const { data , ...pathData } = fileData;
        const currentPath = format2({
            ...pathData,
            dir: path2
        });
...
function clean1(path2) {
    return emptyDir(path2);
}
async function addNotice1(fileData, notice) {
    const data = new Uint8Array(notice.length);
    const reader1 = new StringReader(notice);
    await reader1.read(data);
...
async function logTime1(task) {
    const getTime = ()=>format3(new Date(), "HH:mm:ss")
    ;
    const now = performance.now();
    const startTime = getTime();
    console.log(`[${gray(startTime)}] Starting '${rgb24(task.name, blue)}'`);
    await task.callback();
    const end = performance.now();
    const endTime = getTime();
    const duration = (end - now) / 1000;
    console.log(`[${gray(endTime)}] Finished '${rgb24(task.name, blue)}' after ${rgb24(duration.toFixed(3), purple)} s`);
}
export { clean1 as clean, dest1 as dest, src1 as src, addNotice1 as addNotice };
export { logTime1 as logTime };
